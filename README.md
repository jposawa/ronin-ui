...
`react` and `react-dom` are peer dependencies at `>=18` — React 18 and 19 both work. The package
is **ESM only**; `require()` will not resolve it.
...
- **No global state.** Props in, callbacks out. Nothing to configure, nothing to mount.
- **The container owns layout.** Components set no outer margin and no width of their own.
- **Accessibility is not optional.** Roles, names and keyboard behaviour ship with the
  component, and are what the tests assert.
- **Nothing is imposed on your page.** The only reset is `box-sizing`, scoped to the library's
  own elements.
